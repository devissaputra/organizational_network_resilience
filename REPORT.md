...
At each removal level, 200 same-count pseudo-random comparison sets are generated deterministically. Nodes are ranked by a SHA-256 digest of the release seed, removal level, draw index, and node ID, and the first k nodes are selected. This makes the comparator draw sets stable across supported Python runtimes. The release seed is 20260925.
...
**Comparator validity.** The comparator distribution contains 200 deterministic pseudo-random same-count draws generated by the released SHA-256 ranking rule. It is not an exhaustive enumeration and its p05 and p95 values are descriptive order-statistic diagnostics.
...
**Reproducibility correction.** Strict source-to-release QA found that the earlier comparator depended on Python's `random.sample` implementation, allowing exact comparison draws to drift across runtimes despite a fixed seed. The released v1.1 analysis replaces that mechanism with a SHA-256-ranked deterministic pseudo-random comparator. The targeted-removal curve is unchanged; comparator means, percentile diagnostics, and gaps were regenerated and synchronized from the pinned Zenodo source.
