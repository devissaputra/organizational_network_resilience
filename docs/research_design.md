...
Compare with 200 version-stable deterministic pseudo-random same-count comparator sets at each k. Each set is generated by SHA-256 ranking over the release seed, removal level, draw index, and node ID.
