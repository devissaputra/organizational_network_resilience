...
This study compares static degree-targeted actor loss with a version-stable deterministic pseudo-random comparator distribution generated by SHA-256 ranking and evaluates how the resilience gap changes across increasing stress levels.
...
200 deterministic pseudo-random same-count comparison sets per stress level, generated by SHA-256 ranking over the release seed, removal level, draw index, and node ID.
...
13. 200 deterministic pseudo-random comparison draws rather than exhaustive removal-set enumeration;
